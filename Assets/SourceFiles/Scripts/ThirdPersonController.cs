...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        [Tooltip("Movement multiplier while defending")]
        public float BlockMoveMultiplier = 0.45f;

        [Tooltip("Horizontal speed during the roll")]
        public float RollSpeed = 7.8f;

        [Tooltip("How long the roll lasts")]
        public float RollDuration = 0.48f;

        [Tooltip("Delay before the next roll")]
...
public bool IsRespawning { get; set; } = false;
        ...
        public float RollPoseWeight => RollDuration > 0f ? Mathf.Clamp01(_rollTimer / RollDuration) : 0f;
        public float DoubleJumpPoseWeight => Mathf.Clamp01(_doubleJumpPoseTimer / 0.22f);
        ...
        private void CameraRotation()
{
    ...
    CinemachineCameraTarget.transform.rotation = Quaternion.Euler(
        ...
        private void UpdateActionState()
        {
            ...
            if (_doubleJumpPoseTimer > 0f)
            {
                _doubleJumpPoseTimer -= Time.deltaTime;
            }
            ...
            if (_input.ConsumeRollPressedThisFrame() && _rollCooldownDelta <= 0f && Grounded)
            {
                StartRoll();
                return;
            }
        ...
        {
            if (animationEvent.animatorClipInfo.weight > 0.5f && _controller != null && LandingAudioClip != null)
            {
